...
| `diff-regression` | `Scripts/diff_kotlinc.sh` で kotlinc との出力一致検証（失敗時は TSV と `.artifacts` 相当を Artifact に保存、`DIFF_LOG_PASS=0` で PASS 行省略） |

Swift + LLVM のセットアップは [`.github/actions/setup-swift-llvm`](../.github/actions/setup-swift-llvm/action.yml) に共通化。
