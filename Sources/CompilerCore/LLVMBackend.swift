...
public final class LLVMBackend {
    ...
    private func emitCModule(module: KIRModule, interner: StringInterner) -> String {
        ...
        var lines: [String] = [
            ...
            "  return value;",
            "}",
            "static void kk_coroutine_cancel(intptr_t continuation) {",
            "  KKContinuationState* state = (KKContinuationState*)(void*)continuation;",
            "  if (state) state->isCancelled = 1;",
            "}",
            "static intptr_t kk_coroutine_check_cancellation(intptr_t continuation, intptr_t* outThrown) {",
            "  if (outThrown) { *outThrown = 0; }",
            "  KKContinuationState* state = (KKContinuationState*)(void*)continuation;",
            ...
            "  return 0;",
            "}",
            "static intptr_t kk_is_cancellation_exception(intptr_t throwableRaw) {",
            "  return throwableRaw == 1 ? 1 : 0;",
            "}",
            "static void kk_job_cancel(intptr_t handle) {",
            "  (void)handle;",
            "}",
            ...
            "  KKContinuationState* cs = (KKContinuationState*)(void*)continuation;",
            "  if (cs && cs->isCancelled) return kk_coroutine_suspended();",
            "  if (milliseconds > 0) {",
