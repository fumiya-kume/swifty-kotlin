...
public final class LLVMBackend {
    ...
    private func emitCModule(module: KIRModule, interner: StringInterner) -> String {
        ...
        var lines: [String] = [
            ...
            "  return value;",
            "}",
            "static intptr_t kk_vararg_spread_concat(intptr_t pairCount, ...) {",
            "  va_list ap; va_start(ap, pairCount);",
            "  intptr_t totalLen = 0;",
            "  intptr_t* markers = (intptr_t*)malloc(sizeof(intptr_t) * (size_t)pairCount);",
            "  intptr_t* values  = (intptr_t*)malloc(sizeof(intptr_t) * (size_t)pairCount);",
            "  for (intptr_t i = 0; i < pairCount; i++) {",
            "    markers[i] = va_arg(ap, intptr_t);",
            "    values[i]  = va_arg(ap, intptr_t);",
            "    if (markers[i] == -1) {",
            "      KKArray* src = (KKArray*)(void*)values[i];",
            ...
            "    } else { out->elements[pos++] = values[i]; }",
            "  }",
            "  free(markers); free(values);",
            "  return arr;",
            "}",
