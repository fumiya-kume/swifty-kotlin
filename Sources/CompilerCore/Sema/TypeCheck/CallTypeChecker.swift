...
final class CallTypeChecker {
    ...
    func inferCallExpr(
        ...
        let argTypes = args.enumerated().map { index, argument in
            ...
            // Reuse the already-inferred type for non-lambda args to avoid
            // emitting duplicate diagnostics (e.g. "Unresolved reference") when
            // an argument fails to resolve.
            if let already = inferredNonLambdaArgTypes[index] {
                return already
            }
