...
extension CallTypeChecker {
    ...
    func inferMemberCallImpl(
        ...
        let collectionHOFNames: Set = [
            ...
            "groupBy", "groupingBy", "reduceTo", "sortedBy", "count", "first", "firstOrNull", "last", "find",
        ...
        if isCollectionHOF {
            ...
            case "map", "filter", "filterNot", "filterKeys", "filterValues", "mapNotNull", "forEach", "flatMap", "any", "none", "all",
                 "count", "first", "firstOrNull", "last", "find", "associateBy", "associateWith", "associate",
                ...
                if args.isEmpty {
                    ...
                    case "firstOrNull": resultType = sema.types.makeNullable(collectionElementType)
                    ...
                    case "count": resultType = sema.types.intType
                    case "first", "firstOrNull", "last", "find": resultType = sema.types.makeNullable(collectionElementType)
                    case "associateBy":
                   ...
                   ct.classSymbol == baSymbol
                {
                    ...
                    if calleeStr == "decodeToString" && args.count <= 3 {
                        ...
                        if let chosen = candidates.first(where: { candidate in
                            ...
                            guard sig.receiverType == receiverTypeForCheck else { return false }
                            if sig.parameterTypes.count != args.count { return false }
                            switch args.count {
                            case 0:
                                return true
                            case 1:
                                // Existing charset overload.
                                return charsetExpectedType.map { sig.parameterTypes == [$0] } ?? false
                            case 2:
                                return sig.parameterTypes == [intType, intType]
                            case 3:
                                return sig.parameterTypes == [intType, intType, booleanType]
                            default:
                                return false
                            }
                        }) {
                            ...
                            case 2:
                                _ = driver.inferExpr(args[0].expr, ctx: ctx, locals: &locals, expectedType: intType)
                                _ = driver.inferExpr(args[1].expr, ctx: ctx, locals: &locals, expectedType: intType)
                            case 3:
                                _ = driver.inferExpr(args[0].expr, ctx: ctx, locals: &locals, expectedType: intType)
                                _ = driver.inferExpr(args[1].expr, ctx: ctx, locals: &locals, expectedType: intType)
                                _ = driver.inferExpr(args[2].expr, ctx: ctx, locals: &locals, expectedType: booleanType)
                            default:
                                break
                            }
                            let finalType = safeCall ? sema.types.makeNullable(resultType) : resultType
