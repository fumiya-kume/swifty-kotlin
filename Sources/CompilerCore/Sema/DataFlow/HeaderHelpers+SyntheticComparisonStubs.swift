...
extension DataFlowSemaPhase {
    ...
    /// Registers the top-level `kotlin.comparisons.maxWith(comparator, a, b): T` overload
    /// (and its `minWith` sibling). The 3-arg form takes a `Comparator<in T>` and two
    /// values, returning the greater of the two (or lesser for `minWith`).
    ///
