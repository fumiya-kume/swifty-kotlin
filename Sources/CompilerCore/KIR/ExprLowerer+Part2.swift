...
extension ExprLowerer {
    func lowerExpr(
                ...
                if let sym = sema.symbols.symbol(symbol),
                   ...
                   sema.symbols.parentSymbol(for: symbol) == nil || sema.symbols.symbol(sema.symbols.parentSymbol(for: symbol)!)?.kind == .package {
                    if sym.kind == .property,
                       sema.symbols.delegateStorageSymbol(for: symbol) != nil {
                        // Emit loadGlobal so postProcessTopLevelInitializersAndDelegates
                        // can rewrite this to kk_*_get_value(handle).
        ...
        case .forExpr(_, let iterableExpr, let bodyExpr, let label, _):
            return driver.controlFlowLowerer.lowerForExpr(exprID, iterableExpr: iterableExpr, bodyExpr: bodyExpr, label: label, shared: shared, emit: &instructions)

        case .whileExpr(let conditionExpr, let bodyExpr, let label, _):
            return driver.controlFlowLowerer.lowerWhileExpr(exprID, conditionExpr: conditionExpr, bodyExpr: bodyExpr, label: label, shared: shared, emit: &instructions)

        case .doWhileExpr(let bodyExpr, let conditionExpr, let label, _):
            return driver.controlFlowLowerer.lowerDoWhileExpr(exprID, bodyExpr: bodyExpr, conditionExpr: conditionExpr, label: label, shared: shared, emit: &instructions)
        ...
        case .localFunDecl(let localFunName, let localFunValueParams, _, let localFunBody, _):
            if let symbol = sema.bindings.identifierSymbols[exprID] {
                ...
                var localFunBodyInstructions: KIRLoweringEmitContext = [.beginBlock]
                        ...
                        if let bodyExpr = ast.arena.expr(bodyExprID),
                           case .returnExpr(let value, _, _) = bodyExpr {
                            if let value {
                                let lowered = lowerExpr(
                                    value,
                                    shared: shared, emit: &localFunBodyInstructions
                                )
        ...
        case .localAssign(_, let valueExpr, _):
            ...
            if let symbol = sema.bindings.identifierSymbols[exprID] {
                if sema.symbols.symbol(symbol)?.kind == .backingField {
                    let fieldType = sema.symbols.propertyType(for: symbol) ?? sema.types.anyType
                    let fieldRef = arena.appendExpr(.symbolRef(symbol), type: fieldType)
                    instructions.append(.constValue(result: fieldRef, value: .symbolRef(symbol)))
                    instructions.append(.copy(from: valueID, to: fieldRef))
                ...
                } else if let symInfo = sema.symbols.symbol(symbol), symInfo.kind == .property, {
                    let p = sema.symbols.parentSymbol(for: symbol)
                    let pk = p.flatMap({ sema.symbols.symbol($0) })?.kind
                    return pk == nil || pk == .package
                }() {
                    if sema.symbols.delegateStorageSymbol(for: symbol) != nil {
                        let setterSymbol = SymbolID(rawValue: -13_000 - symbol.rawValue)
                        let setterResult = arena.appendExpr(.temporary(Int32(arena.expressions.count)), type: sema.types.unitType)
                        let calleeName = sema.symbols.symbol(symbol)?.name ?? interner.intern("set")
                        instructions.append(.call(
                            symbol: setterSymbol,
                            callee: calleeName,
                            arguments: [valueID],
        ...
        case .indexedAccess(let receiverExpr, let indices, _):
            return driver.callLowerer.lowerIndexedAccessExpr(exprID, receiverExpr: receiverExpr, indices: indices, shared: shared, emit: &instructions)

        case .indexedAssign(let receiverExpr, let indices, let valueExpr, _):
            return driver.callLowerer.lowerIndexedAssignExpr(exprID, receiverExpr: receiverExpr, indices: indices, valueExpr: valueExpr, shared: shared, emit: &instructions)
        ...
        case .ifExpr(let condition, let thenExpr, let elseExpr, _):
            return driver.controlFlowLowerer.lowerIfExpr(exprID, condition: condition, thenExpr: thenExpr, elseExpr: elseExpr, shared: shared, emit: &instructions)

        case .tryExpr(let bodyExpr, let catchClauses, let finallyExpr, _):
            return driver.controlFlowLowerer.lowerTryExpr(exprID, bodyExpr: bodyExpr, catchClauses: catchClauses, finallyExpr: finallyExpr, shared: shared, emit: &instructions)

        case .binary(let op, let lhs, let rhs, _):
            return driver.callLowerer.lowerBinaryExpr(exprID, op: op, lhs: lhs, rhs: rhs, shared: shared, emit: &instructions)

        case .call(let calleeExpr, _, let args, _):
            return driver.callLowerer.lowerCallExpr(exprID, calleeExpr: calleeExpr, args: args, shared: shared, emit: &instructions)

        case .memberCall(let receiverExpr, let calleeName, _, let args, _):
            return driver.callLowerer.lowerMemberCallExpr(exprID, receiverExpr: receiverExpr, calleeName: calleeName, args: args, shared: shared, emit: &instructions)
        ...
        case .safeMemberCall(let receiverExpr, let calleeName, _, let args, _):
            return driver.callLowerer.lowerSafeMemberCallExpr(exprID, receiverExpr: receiverExpr, calleeName: calleeName, args: args, shared: shared, emit: &instructions)
        ...
        case .indexedCompoundAssign(_, let receiverExpr, let indices, let valueExpr, _):
            return driver.callLowerer.lowerIndexedCompoundAssignExpr(exprID, receiverExpr: receiverExpr, indices: indices, valueExpr: valueExpr, shared: shared, emit: &instructions)
        ...
        case .whenExpr(let subject, let branches, let elseExpr, _):
            return driver.controlFlowLowerer.lowerWhenExpr(exprID, subject: subject, branches: branches, elseExpr: elseExpr, shared: shared, emit: &instructions)
        ...
        case .inExpr(let lhsExpr, let rhsExpr, _):
            let lhsID = lowerExpr(
                lhsExpr, shared: shared, emit: &instructions
            )
            let rhsID = lowerExpr(
                rhsExpr, shared: shared, emit: &instructions
            )
            let result = arena.appendExpr(.temporary(Int32(arena.expressions.count)), type: boundType ?? boolType)
            instructions.append(.call(
                symbol: nil,
        ...
        case .destructuringDecl(let names, _, let initializer, _):
            ...
            let rhsID = lowerExpr(
                initializer, shared: shared, emit: &instructions
            )
