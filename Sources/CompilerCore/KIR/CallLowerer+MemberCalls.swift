...
extension CallLowerer {
    ...
    private func lowerMemberLikeCallExpr(
        ...
        // Object member property read: load from global slot instead
        // of emitting a function call to the property name.
        if args.isEmpty {
            let chosenSym = sema.bindings.callBindings[exprID]?
                .chosenCallee
            let valueSym = chosenSym
                ?? sema.bindings.identifierSymbol(for: exprID)
            if let valueSym,
               let info = sema.symbols.symbol(valueSym),
               info.kind == .property,
               let parent = sema.symbols.parentSymbol(
                   for: valueSym
               ),
               sema.symbols.symbol(parent)?.kind == .object
            {
                let propType = sema.bindings.exprTypes[exprID]
                    ...
                    ?? sema.types.anyType
                let id = arena.appendExpr(
                    .symbolRef(valueSym), type: propType
                )
                instructions.append(
                    .loadGlobal(result: id, symbol: valueSym)
                )
                return id
            }
        }
