...
extension CollectionLiteralLoweringPass {
    ...
    // numeric ranges share the same RuntimeRangeBox (Int, i.e. 64-bit) layout.
    // UInt values fit in the non-negative half of Int64, so signed comparisons
    // are correct. ULong values > Int64.max are stored via bit-pattern
    // reinterpretation and may produce incorrect results — this is a known
    // limitation until unsigned comparison helpers are added to the runtime.
