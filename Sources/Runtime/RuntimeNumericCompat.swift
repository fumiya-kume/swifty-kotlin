...
// MARK: - STDLIB-500..513: Float overloads for trig / rounding / sqrt
...
// MARK: - STDLIB-510..511: roundToInt / roundToLong extension functions

@_cdecl("kk_float_roundToInt")
public func kk_float_roundToInt(_ value: Int) -> Int {
    let f = kk_bits_to_float(value)
    if f.isNaN { return 0 }
    // Kotlin Math.round(): ties toward positive infinity = floor(x + 0.5)
...
// MARK: - STDLIB-512..513: ulp / nextUp / nextDown extension properties
