...
public func kk_kxmini_launch_with_exception_handler(_ entryPointRaw: Int, _ functionID: Int, _ handlerRaw: Int) -> Int {
    ...
    enum LaunchExceptionHandler {
        case legacy(RuntimeExceptionHandlerBox)
        case context(RuntimeCoroutineContextExceptionHandlerBox, contextHandle: Int)

        func invoke(_ exception: Int) {
            switch self {
            case let .legacy(handler):
                handler.handler(exception)
            case let .context(handler, contextHandle):
                handler.invoke(context: contextHandle, exception: exception)
            }
        }
    }

    // Resolve exception handler.
    var exceptionHandler: LaunchExceptionHandler?
    if handlerRaw != 0 {
        if let box = contextBoxFromHandle(handlerRaw), let handler = box.exceptionHandler {
            exceptionHandler = .context(handler, contextHandle: handlerRaw)
        } else if let ptr = UnsafeMutableRawPointer(bitPattern: handlerRaw) {
            let isObjPointer = runtimeStorage.withLock { state in
                state.objectPointers.contains(UInt(bitPattern: ptr))
            }
            if isObjPointer,
               let handler = Unmanaged<AnyObject>.fromOpaque(ptr).takeUnretainedValue() as? RuntimeExceptionHandlerBox
            {
                exceptionHandler = .legacy(handler)
            }
        }
    }
    ...
    KxMiniRuntime.launch {
        ...
        if thrownException != 0, let handler = exceptionHandler {
            handler.invoke(thrownException)
...
///
/// STDLIB-CORO-077: Also handles CoroutineContext boxes. If dispatcherRaw
/// is a pointer to a CoroutineContext box, the dispatcher is extracted from it
...
public func kk_with_context(_ dispatcherRaw: Int, _ blockFnPtr: Int, _ continuation: Int) -> Int {
    // STDLIB-CORO-077: If dispatcherRaw is a CoroutineContext box, delegate
    // to kk_with_context_full which handles context element propagation.
    if contextBoxFromHandle(dispatcherRaw) != nil {
...
private func contextBoxFromHandle(_ handle: Int) -> RuntimeCoroutineContextBox? {
    ...
    let box = Unmanaged<RuntimeCoroutineContextBox>.fromOpaque(ptr).takeUnretainedValue()
    let snapshot = RuntimeCoroutineContextBox(
        dispatcherTag: box.dispatcherTag,
        name: box.name,
        exceptionHandler: box.exceptionHandler
    )
    contextBoxStorageLock.unlock()
    return snapshot
}
...
public func kk_with_context_full(_ contextHandle: Int, _ blockFnPtr: Int, _ continuation: Int) -> Int {
    ...
    let dispatcherTag = box?.dispatcherTag ?? 0

    if let contState = runtimeContinuationState(from: continuation),
       let box {
        let previousName = contState.coroutineName
        let previousHandler = contState.exceptionHandler
        defer {
            contState.coroutineName = previousName
            contState.exceptionHandler = previousHandler
        }
