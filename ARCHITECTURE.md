...
| `Codegen/` | LLVM IR 生成 + リンク | `CodegenPass.swift`, `LLVMBackend.swift` (+分割4ファイル), `LLVMCAPIBackend.swift`, `LLVMCAPIBindings.swift` (+分割3ファイル), `NativeEmitter.swift` (+分割2ファイル), `LinkPass.swift`, `NameMangler.swift`, `CodegenBackend.swift` | コード生成バグ、ABI修正、リンクエラー |
