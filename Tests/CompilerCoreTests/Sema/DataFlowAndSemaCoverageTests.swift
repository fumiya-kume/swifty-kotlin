...
final class DataFlowAndSemaCoverageTests: XCTestCase {
    ...
    // MARK: - Block-parsed local decl: uninitialized var with type annotation

    func testBlockParsedUninitializedVarWithTypeAnnotation() throws {
        let source = """
        fun main() {
            val result = run {
                var x: Int
    ...
    // MARK: - Block-parsed local decl: typed val with initializer

    func testBlockParsedTypedValWithInitializer() throws {
        let source = """
        fun main() {
            val result = run {
                val x: Int = 10
    ...
    // MARK: - Block-parsed local decl: inferred val (regression)

    func testBlockParsedInferredValStillWorks() throws {
        let source = """
        fun main() {
            val result = run {
                val x = 99
    ...
    // MARK: - Block-parsed local decl: use-before-init in block

    func testBlockParsedUseBeforeInitEmitsDiagnostic() throws {
        let source = """
        fun main(): Int {
            var x: Int
            return x
    ...
    // MARK: - When-expr init propagation: exhaustive when initializes variable

    func testWhenExprExhaustiveInitPropagation() throws {
        let source = """
        fun main(): Int {
            val x = 1
            var y: Int
    ...
    // MARK: - When-expr init propagation: non-exhaustive when does NOT propagate

    func testWhenExprNonExhaustiveDoesNotPropagateInit() throws {
        let source = """
        fun main(): Int {
            val x = 1
            var y: Int
    ...
    // MARK: - When-expr init propagation: boolean exhaustive when initializes variable

    func testWhenExprBooleanExhaustiveInitPropagation() throws {
        let source = """
        fun main(): Int {
            val flag = true
            var y: Int
