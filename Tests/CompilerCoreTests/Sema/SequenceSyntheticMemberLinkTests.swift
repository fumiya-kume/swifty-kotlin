...
final class SequenceSyntheticMemberLinkTests: XCTestCase {
    ...
    func testSequenceZipResolvesInCallExpressions() throws {
        let source = """
        fun zipValuesSize(): Int {
            val left = sequenceOf(1, 2, 3)
            val right = sequenceOf("a", "b", "c")
            return left.zip(right).toList().size
        }
        """
        ...
        try withTemporaryFile(contents: source) { path in
            ...
            XCTAssertFalse(
                ctx.diagnostics.hasError,
                "Expected Sequence.zip surface to resolve cleanly, got: \(diagnosticSummary)"
            )

            let sema = try XCTUnwrap(ctx.sema)
            let memberFQName = ["kotlin", "sequences", "Sequence", "zip"]
                .map { ctx.interner.intern($0) }
            let links = Set(
                sema.symbols.lookupAll(fqName: memberFQName)
                    .compactMap { sema.symbols.externalLinkName(for: $0) }
            )
            XCTAssertTrue(links.contains("kk_sequence_zip"))

            let zipSymbol = try XCTUnwrap(sema.symbols.lookup(fqName: memberFQName))
            let signature = try XCTUnwrap(sema.symbols.functionSignature(for: zipSymbol))
            guard case let .classType(returnSequenceType) = sema.types.kind(of: signature.returnType),
                  let returnElementProjection = returnSequenceType.args.first,
                  case let .out(returnElementType) = returnElementProjection,
                  case let .classType(pairType) = sema.types.kind(of: returnElementType)
            else {
                return XCTFail("Expected Sequence.zip() to return Sequence<Pair<T, R>>")
            }
            XCTAssertEqual(
                try ctx.interner.resolve(XCTUnwrap(sema.symbols.symbol(returnSequenceType.classSymbol)?.name)),
                "Sequence"
            )
            XCTAssertEqual(
    ...
    func testSequenceAverageOfResolvesInCallExpressions() throws {
        let source = """
        fun averageLengths(): Double {
            val values = sequenceOf("a", "bbb", "cc")
            return values.averageOf { value ->
                value.length
            }
        }
        """
        ...
        try withTemporaryFile(contents: source) { path in
            ...
            XCTAssertFalse(
                ctx.diagnostics.hasError,
                "Expected Sequence.averageOf surface to resolve cleanly, got: \(diagnosticSummary)"
            )

            let sema = try XCTUnwrap(ctx.sema)
            let memberFQName = ["kotlin", "sequences", "Sequence", "averageOf"]
                .map { ctx.interner.intern($0) }
            let links = Set(
                sema.symbols.lookupAll(fqName: memberFQName)
                    .compactMap { sema.symbols.externalLinkName(for: $0) }
            )
            XCTAssertTrue(links.contains("kk_sequence_averageOf"))
        }
    }
