...
final class ListSyntheticMemberLinkTests: XCTestCase {
    ...
    func testSequenceConstrainOnceUsesRuntimeExternalLink() throws {
        let source = """
        fun render(values: Sequence<Int>): Sequence<Int> = values.constrainOnce()
        """
            ...
            let symbol = try XCTUnwrap(
                sema.symbols.lookup(
                    fqName: [
                        ...
                        ctx.interner.intern("sequences"),
                        ctx.interner.intern("Sequence"),
                        ctx.interner.intern("constrainOnce"),
                    ]
                ),
                "Expected synthetic Sequence.constrainOnce member to be registered"
            )
            XCTAssertEqual(sema.symbols.externalLinkName(for: symbol), "kk_sequence_constrainOnce")
        }
    }

    func testSequenceChunkedTransformUsesRuntimeExternalLink() throws {
        let source = """
        fun render(values: Sequence<Int>) {
            println(values.chunked(2) { chunk -> chunk.size }.toList())
        }
        """
        ...
        try withTemporaryFile(contents: source) { path in
            ...
            let ast = try XCTUnwrap(ctx.ast)
            let sema = try XCTUnwrap(ctx.sema)
            let chunkedCall = try XCTUnwrap(firstExprID(in: ast) { _, expr in
                guard case let .memberCall(_, callee, _, _, _) = expr else { return false }
                return ctx.interner.resolve(callee) == "chunked"
            })
            let chosenCallee = try XCTUnwrap(sema.bindings.callBinding(for: chunkedCall)?.chosenCallee)
            XCTAssertEqual(
                sema.symbols.externalLinkName(for: chosenCallee),
    ...
    func testSequenceWindowedTransformUsesRuntimeExternalLink() throws {
        let source = """
        fun render(values: Sequence<Int>) {
            println(values.windowed(3, 2, true) { window -> window.size }.toList())
        }
        """
        ...
        try withTemporaryFile(contents: source) { path in
            ...
            let ast = try XCTUnwrap(ctx.ast)
            let sema = try XCTUnwrap(ctx.sema)
            let windowedCall = try XCTUnwrap(firstExprID(in: ast) { _, expr in
                guard case let .memberCall(_, callee, _, _, _) = expr else { return false }
                return ctx.interner.resolve(callee) == "windowed"
            })
            let chosenCallee = try XCTUnwrap(sema.bindings.callBinding(for: windowedCall)?.chosenCallee)
            XCTAssertEqual(
                sema.symbols.externalLinkName(for: chosenCallee),
