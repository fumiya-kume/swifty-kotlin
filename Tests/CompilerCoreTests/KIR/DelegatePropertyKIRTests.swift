...
final class DelegatePropertyKIRTests: XCTestCase {
    ...
    func testObservableSetterRewritesToSetValueCall() throws {
        ...
        try withTemporaryFile(contents: source) { path in
            ...
            XCTAssertFalse(ctx.diagnostics.hasError,
                           "observable setter should compile without errors: \(ctx.diagnostics.diagnostics.map(\.message))")

            let module = try XCTUnwrap(ctx.kir)
            let mainBody = try findKIRFunctionBody(named: "main", in: module, interner: ctx.interner)
            let callees = extractCallees(from: mainBody, interner: ctx.interner)

            XCTAssertTrue(callees.contains("kk_observable_set_value"),
                          "Assignment to observable delegate should emit kk_observable_set_value, got: \(callees)")
        }
    }
    ...
    func testVetoableSetterRewritesToSetValueCall() throws {
        ...
        try withTemporaryFile(contents: source) { path in
            ...
            XCTAssertFalse(ctx.diagnostics.hasError,
                           "vetoable setter should compile without errors: \(ctx.diagnostics.diagnostics.map(\.message))")

            let module = try XCTUnwrap(ctx.kir)
            let mainBody = try findKIRFunctionBody(named: "main", in: module, interner: ctx.interner)
            let callees = extractCallees(from: mainBody, interner: ctx.interner)

            XCTAssertTrue(callees.contains("kk_vetoable_set_value"),
                          "Assignment to vetoable delegate should emit kk_vetoable_set_value, got: \(callees)")
        }
    }
