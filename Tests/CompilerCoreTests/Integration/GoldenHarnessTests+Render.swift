...
extension GoldenHarnessTests {
    ...
    func renderExpr(_ expr: Expr, interner: StringInterner) -> String {
        ...
        case let .localFunDecl(name, typeParams, valueParams, returnType, body, _):
            let typeParamsStr = typeParams.map { interner.resolve($0.name) }.joined(separator: ",")
            ...
            let typeParamsPrefix = typeParams.isEmpty ? "" : " typeParams=[\(typeParamsStr)]"
            return "localFunDecl \(interner.resolve(name))\(typeParamsPrefix) params=[\(params)] returnType=\(retStr) body=\(bodyStr)"
