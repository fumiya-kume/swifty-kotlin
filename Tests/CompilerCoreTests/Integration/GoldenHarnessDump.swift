...
/// Compiler phases are not thread-safe; Swift Testing may schedule golden cases in parallel across suites.
private enum GoldenHarnessDumpSynchronization {
    static let lock = NSLock()

    static func isolated<R>(_ work: () throws -> R) rethrows -> R {
        lock.lock()
        defer { lock.unlock() }
        return try work()
    }
...
enum GoldenHarnessDump {
    static func dumpLexer(sourcePath: String) throws -> String {
        try GoldenHarnessDumpSynchronization.isolated {
