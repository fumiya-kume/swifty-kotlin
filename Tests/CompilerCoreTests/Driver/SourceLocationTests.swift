...
final class SourceLocationTests: XCTestCase {
    ...
    func testVisibilityFromRawValue() {
        ...
        XCTAssertNil(Visibility(rawValue: 99))
    }

    // MARK: - Modifiers

    func testModifiersSingleBitValues() {
        XCTAssertEqual(Modifiers.public.rawValue, 1 << 0)
        XCTAssertEqual(Modifiers.internal.rawValue, 1 << 1)
        XCTAssertEqual(Modifiers.private.rawValue, 1 << 2)
        XCTAssertEqual(Modifiers.protected.rawValue, 1 << 3)
        XCTAssertEqual(Modifiers.final.rawValue, 1 << 4)
        XCTAssertEqual(Modifiers.open.rawValue, 1 << 5)
        XCTAssertEqual(Modifiers.abstract.rawValue, 1 << 6)
        XCTAssertEqual(Modifiers.sealed.rawValue, 1 << 7)
        XCTAssertEqual(Modifiers.data.rawValue, 1 << 8)
        XCTAssertEqual(Modifiers.annotationClass.rawValue, 1 << 9)
        XCTAssertEqual(Modifiers.inline.rawValue, 1 << 10)
        XCTAssertEqual(Modifiers.suspend.rawValue, 1 << 11)
        XCTAssertEqual(Modifiers.tailrec.rawValue, 1 << 12)
        XCTAssertEqual(Modifiers.operator.rawValue, 1 << 13)
        XCTAssertEqual(Modifiers.infix.rawValue, 1 << 14)
        XCTAssertEqual(Modifiers.crossinline.rawValue, 1 << 15)
        XCTAssertEqual(Modifiers.noinline.rawValue, 1 << 16)
        XCTAssertEqual(Modifiers.vararg.rawValue, 1 << 17)
        XCTAssertEqual(Modifiers.external.rawValue, 1 << 18)
        XCTAssertEqual(Modifiers.expect.rawValue, 1 << 19)
        XCTAssertEqual(Modifiers.actual.rawValue, 1 << 20)
        XCTAssertEqual(Modifiers.value.rawValue, 1 << 21)
        XCTAssertEqual(Modifiers.enumModifier.rawValue, 1 << 22)
    }

    func testModifiersUnion() {
        let combined: Modifiers = [.public, .final, .suspend]
        XCTAssertTrue(combined.contains(.public))
        XCTAssertTrue(combined.contains(.final))
        XCTAssertTrue(combined.contains(.suspend))
        XCTAssertFalse(combined.contains(.private))
        XCTAssertFalse(combined.contains(.abstract))
    }

    func testModifiersIntersection() {
        let a: Modifiers = [.public, .final, .inline]
        let b: Modifiers = [.final, .inline, .suspend]
        let intersection = a.intersection(b)
        XCTAssertTrue(intersection.contains(.final))
        XCTAssertTrue(intersection.contains(.inline))
        XCTAssertFalse(intersection.contains(.public))
        XCTAssertFalse(intersection.contains(.suspend))
    }

    func testModifiersEmpty() {
        let empty: Modifiers = []
