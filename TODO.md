...
- [x] STDLIB-IO-PATH-FN-021: `getAttribute` 関数の実装
- [x] STDLIB-IO-PATH-FN-022: `getLastModifiedTime` 関数の実装
- [x] STDLIB-IO-PATH-FN-023: `getOwner` 関数の実装
- [x] STDLIB-IO-PATH-FN-024: `getPosixFilePermissions` 関数の実装
- [x] STDLIB-IO-PATH-FN-025: `inputStream` 関数の実装
- [x] STDLIB-IO-PATH-FN-026: `moveTo` 関数の実装
...
- [x] STDLIB-SEQ-FN-027: `filterIsInstanceTo` 関数の実装
...
- [x] STDLIB-SYSTEM-FN-007: `measureTimeMillis` 関数の実装
- [x] STDLIB-SYSTEM-FN-001: `exitProcess` 関数の実装
- [x] STDLIB-SYSTEM-FN-002: `getTimeMicros` 関数の実装
- [x] STDLIB-SYSTEM-FN-003: `getTimeMillis` 関数の実装
- [x] STDLIB-SYSTEM-FN-004: `getTimeNanos` 関数の実装
...
- [x] STDLIB-TEXT-TYPE-009: `MatchNamedGroupCollection` インターフェースの実装
...
- [x] STDLIB-TEXT-FN-017: `format` 関数の実装
- [x] STDLIB-TEXT-FN-018: `get` 関数の実装
...
- [x] STDLIB-TEXT-FN-036: `lineSequence` 関数の実装
- [x] STDLIB-TEXT-FN-037: `lines` 関数の実装
...
- [x] STDLIB-TEXT-FN-053: `removeSurrounding` 関数の実装
- [x] STDLIB-TEXT-FN-054: `repeat` 関数の実装
- [x] STDLIB-TEXT-FN-055: `replace` 関数の実装
- [ ] STDLIB-TEXT-FN-056: `replaceAfter` 関数の実装
- [x] STDLIB-TEXT-FN-057: `replaceAfterLast` 関数の実装
- [x] STDLIB-TEXT-FN-058: `replaceBefore` 関数の実装
- [x] STDLIB-TEXT-FN-059: `replaceBeforeLast` 関数の実装
...
- [x] STDLIB-TEXT-FN-066: `single` 関数の実装
- [x] STDLIB-TEXT-FN-067: `singleOrNull` 関数の実装
- [x] STDLIB-TEXT-FN-068: `slice` 関数の実装
...
- [x] STDLIB-TEXT-FN-076: `substringBefore` 関数の実装
...
- [x] STDLIB-TEXT-FN-113: `trimMargin` 関数の実装
...
- [x] STDLIB-TIME-FN-011: `toKotlinInstant` 関数の実装
- [ ] STDLIB-TIME-FN-012: `toTimeUnit` 関数の実装

#### kotlin.uuid 型の実装
- [x] STDLIB-UUID-TYPE-002: `Uuid` クラスの実装

#### kotlin.uuid 関数の実装
- [x] STDLIB-UUID-FN-001: `getUuid` 関数の実装
...
- [x] STDLIB-COMP-FN-033: `minOf` 関数の実装（Byte版、3引数）
...
- [x] STDLIB-COMP-FN-037: `minOf` 関数の実装（Double版、vararg）
- [x] STDLIB-COMP-FN-038: `minOf` 関数の実装（Float版、2引数）
...
- [x] STDLIB-WASM-UNSAFE-FN-001: `withScopedMemoryAllocator(block)` を追加する
...
- [x] STDLIB-CINTEROP-TYPE-021: `kotlinx.cinterop.CPrimitiveVar` sealed class surface を追加する
...
- [x] STDLIB-CINTEROP-TYPE-029: `kotlinx.cinterop.NativePlacement` class shape を公式 surface と整合させる
...
- [x] STDLIB-CINTEROP-FN-013: `refTo(index)` を追加する
...
- [x] TEST-SEQ-010: `kotlin.sequences` 既存関数のエッジケースを拡充する。`distinctBy`（空・全要素同一キー・キーセレクタ例外伝播）、`filterIsInstance`（空・全一致・全不一致）、`reduceIndexed` / `reduceRightIndexed`（単一要素で accumulator 未呼出）、および中間操作の遅延評価回数の検証（`RuntimeSequenceTests+BuilderAndAdvanced.swift` の `_lazyTestYieldCounter` 機構を活用）
- [x] TEST-COMP-011: `kotlin.comparisons` の Comparator 合成を補強する。`naturalOrder` / `reverseOrder` のトランポリンに `runtimeNullSentinelInt` を渡したときの挙動、`compareBy` で全キー等値のとき `0` を返すこと、参照型オブジェクトの厳密な安定ソート（同値要素の原順序保持をインデックスベースで検証）。既存 `RuntimeComparatorTests.swift` は充実しているため上記の隙間に限定する
- [x] TEST-COL-012: `kotlin.collections` の `Set` 高階関数の Runtime/Codegen テストを追加する。`kk_set_filter` / `filterNot` / `map` / `flatMap` / `all` / `any` / `first` / `last` / `lastOrNull` / `maxOrNull` / `minOrNull` / `sorted` / `sortedDescending` / `singleOrNull` / `count{}`（`kk_set_count_predicate`）/ `forEach` は実装の実体が `RuntimeCollectionHOF.swift` にあるが、Runtime テストも Codegen 統合テストも存在しない（Set 専用テストファイルが皆無）。カバー対象: 空 Set・単一要素・全一致/全不一致・要素順序・`first`/`last` の空 Set で例外。`none` と `mapNotNull` は既存カバー済みのため対象外
- [x] TEST-COL-013: `kotlin.collections` の `Map` 高階関数 `getOrDefault` / `flatMap` / `mapNotNull` / `maxByOrNull` / `minByOrNull` の Codegen 統合テストを追加する（`RuntimeSetAndMap.swift` 等に実装ありだが実行テストなし）。カバー対象: 空 Map・キー不在時の `getOrDefault` デフォルト返却・全エントリ変換・`maxByOrNull`/`minByOrNull` の空 Map で `null`
- [x] TEST-COL-014: `kotlin.collections` の `List` 受信者版 `reduceIndexedOrNull` / `scanIndexed` の Codegen 統合テストを追加する。Sequence 受信者版はカバー済みだが List 受信者の実行テストが欠落。カバー対象: 空（`reduceIndexedOrNull` は `null`、`scanIndexed` は initial のみ）・単一要素・accumulator に渡る index の検証
