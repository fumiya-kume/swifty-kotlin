...
- [x] P5-22: 式パーサ/型推論を Kotlin 基本演算子セットへ拡張する（spec.md J5/J6/J9）
  - [x] unary 演算（`!`/unary `+`/unary `-`）と優先順位を実装する
  - [x] 比較/論理/型演算子（`!=`/`<`/`<=`/`>`/`>=`/`&&`/`||`/`is`/`as`）と優先順位を実装する
    - [x] 比較/論理（`!=`/`<`/`<=`/`>`/`>=`/`&&`/`||`）を Parser/Sema/KIR/backend に接続
    - [x] 型演算子（`is`/`as`）を Parser/Sema/KIR/backend に接続
  - [x] Elvis（`?:`）/ null 断定（`!!`）/ safe call（`?.`）を AST と型推論に反映する
  - [x] assignment（`=` と複合代入）を式/文として扱い、`val` 再代入診断と接続する
...
- [x] P5-24: CST 粒度を spec 最低ラインへ引き上げる（spec.md J5.1）
  - [x] `importList` ノードを parser で実生成し、top-level 構造を明確化する
  - [x] `ifExpr`/`whenExpr`/`tryExpr`/`callExpr` など主要構文の SyntaxKind を CST で直接生成する
  - [x] AST builder の「statement 再パース」依存を段階的に削減する

- [x] P5-25: 動的ディスパッチ（vtable/itable）を call lowering/codegen に接続する（spec.md J13.2）
  - [x] member call で virtual dispatch が必要なケースを判定し、slot ベース呼び出し IR を導入する
  - [x] class override は同一 vtable slot、interface call は itable 経由で解決する
  - [x] override/interface dispatch の E2E 回帰ケースを追加する

- [x] P5-26: ABILowering の boxing/unboxing を実装する（spec.md J12.1/J13.3）
  - [x] nullable primitive と `Any?` 境界で boxing/unboxing 規則を実装する
  - [x] runtime 値表現（tagged/boxed）と `kk_println_any`/call ABI の整合を取る
  - [x] boxing 境界を含む型推論 + 実行回帰ケースを追加する

- [x] P5-27: runtime C ABI シグネチャを spec 固定形へ厳密化する（spec.md J16.1）
  - [x] `kk_alloc` など公開シンボルの引数/戻り型を仕様シグネチャへ揃える（nullable 許容の方針も含む）
  - [x] compiler/backend 側の extern 宣言を runtime 実体と自動突合する仕組み（共有 header 生成など）を導入する
  - [x] ABI ずれを検出するビルド時テストを追加する

- [x] P5-28: `interface` 宣言を `class` へ潰さずに front-to-back で区別する（spec.md J6/J7/J13.2）
  - [x] AST/Sema モデルで interface 種別を保持し、`FrontendPhases` の `interfaceDecl -> classDecl` 変換を解消する
  - [x] header 収集・継承グラフ・layout 合成で interface を正しく扱う（superClass と interface 集合を分離）
  - [x] interface 実装/override/dispatch の差分ケースを追加する

- [x] P5-29: class/object body の member 宣言（fun/property）を AST/Sema/KIR に接続する（spec.md J6/J7/J12）
  - [x] `ClassDecl`/`ObjectDecl` に member 宣言リストを保持し、body 内 `fun`/`val`/`var` を AST へ昇格する
  - [x] nested `class`/`object` 宣言を class/object body から保持し、owner FQName 配下へシンボル化する
  - [x] `ClassMemberScope` を使った member symbol 収集と参照解決（`this` レシーバ込み）を実装する
  - [x] backing field 実体化と delegated property（`by`）の lowering を実装する
  - [x] member 関数/プロパティを KIR へ落とし込み、metadata/layout と整合させる

- [x] P5-30: constructor 宣言と呼び出し解決を実装する（spec.md J5/J7/J13）
  - [x] primary/secondary constructor の構文を Parser/AST で扱い、`constructor` symbol を定義する
  - [x] class 呼び出しを constructor overload 解決へ接続し、`super`/`this` 委譲規則を検査する
  - [x] init block と constructor 実行順序を lowering/codegen で保証する

- [x] P5-31: 型参照を named-only から拡張し、型引数・関数型・明示型引数呼び出しを実装する（spec.md J5/J6/J8/J9）
  - [x] `TypeRef` を type arguments/projection(`in`/`out`/`*`)/function type/suspend function type 対応へ拡張する
  - [x] `resolveTypeRef` で `ClassType.args` と `FunctionType` を復元し、variance 検査へ接続する
  - [x] 継承節（`superTypes`）で型引数付き supertype を保持し、継承グラフ構築時に型実引数を失わないようにする
  - [x] call-site 明示型引数（`foo<Int>(...)`）を parser/sema/resolver に通し、推論結果と統合する

- [x] P5-32: typealias の実体型保存と展開解決を実装する（spec.md J6/J7/J8/J14.3）
  - [x] `TypeAliasDecl` に RHS 型参照を保持し、Parser/AST builder で抽出する
  - [x] Sema に alias target 解決と循環検出を追加し、型解決時に展開する
  - [x] metadata export/import で typealias シグネチャを保存/復元し、不整合診断を追加する
...
- [x] P5-38: 型パラメータ境界（`T : Upper` / `where` 句）を Parser/Sema/Resolver に実装する（spec.md J6/J8/J9）
  - [x] `TypeParamDecl` に upper bound 情報を保持し、宣言ヘッダで `where` 句を抽出する
  - [x] 型推論/overload 解決で type argument が境界制約を満たすことを検証する
  - [x] 境界違反時の診断と回帰ケース（宣言側・呼び出し側）を追加する

- [x] P5-39: vararg 実引数を call lowering/ABI で正規化する（spec.md J9/J11/J12）
  - [x] 複数実引数が 1 つの vararg parameter に束縛された場合に、KIR で配列（または等価表現）へパックする
  - [x] spread 引数（`*args`）と通常引数の混在を lowering で保持し、callee 側 ABI と整合させる
  - [x] vararg + default 引数 + named 引数の組み合わせ回帰ケースを追加する

- [x] P5-40: 未解決参照・型参照の診断を strict 化する（spec.md J7/J8/J9）
  - [x] `nameRef`/`call`/`memberCall` の unresolved 経路で `errorType` を返すだけでなく診断（`KSWIFTK-SEMA-*`）を出す
  - [x] `resolveTypeRef` の未知型 fallback（`Any` へ丸める現実装）を廃止し、型名解決失敗を診断する
  - [x] unresolved 診断の回帰ケース（識別子・関数呼び出し・型注釈）を追加する

- [x] P5-41: local 変数宣言の完全化（未初期化宣言・型注釈）を実装する（spec.md J6/J7/J9）
  - [x] AST `localDecl` が initializer 省略を表現できるよう拡張する（`var x: Int` 対応）
  - [x] local 宣言の型注釈（`val x: T = ...` / `var x: T`）を保持し、推論/代入検査に接続する
  - [x] 未初期化 local の使用前参照を診断する data-flow チェックを追加する

- [x] P5-42: block-scope 宣言（local function など）を AST/Sema/KIR に接続する（spec.md J6/J7/J9）
  - [x] block 内 `fun` 宣言を `blockExpressions` で捨てない AST 表現へ変更する
  - [x] `FunctionScope`/`BlockScope` に local function symbol を登録し、後続式から参照解決できるようにする
  - [x] local function 呼び出しの KIR 生成と回帰ケースを追加する

- [x] P5-43: import 解決を library symbol まで拡張し wildcard/default import を有効化する（spec.md J7）
  - [x] library import 時に package symbol と package->top-level symbol インデックスを構築する
  - [x] `buildFileScopes` が source 由来だけでなく imported symbol を default/wildcard import 対象へ取り込むよう修正する
  - [x] `import foo.bar.*` と default import が `.kklib` シンボルで機能する回帰ケースを追加する

- [x] P5-44: coroutine launcher lowering の zero-argument 制約を解消する（spec.md J17.3）
  - [x] `runBlocking`/`launch`/`async` が 0 引数 suspend 関数参照専用となっている制約を解除する
  - [x] suspend lambda/closure と引数付き suspend 関数呼び出しへの橋渡しを実装する
  - [x] launcher + 引数あり suspend 関数の E2E 回帰ケースを追加する

- [x] P5-45: `-g` の debug info 出力を backend/link へ実装する（spec.md J1/J15）
  - [x] `CompilerOptions.emitsDebugInfo` を `LLVMBackend`/`LLVMCAPIBackend` の emit path へ反映する
  - [x] synthetic C backend の clang 呼び出しに debug flag を接続し、出力に debug section が含まれることを確認する
  - [x] LLVM C API backend で DI metadata の最小生成（または仕様化した制限つき対応）を追加する

- [x] P5-46: 数値リテラルの lexical grammar を Kotlin 仕様に合わせる（spec.md J4）
  - [x] `0o` など Kotlin 非対応プレフィックスを受理しないよう lexer を修正する
  - [x] underscore/suffix の許容位置を仕様どおりに検証し、違反時診断を固定する
  - [x] 数値リテラル grammar 回帰ケース（valid/invalid）を golden に追加する

- [x] P5-47: block expression の文脈で複文評価（複数 statement + 末尾式）を実装する（spec.md J6/J9/J11）
  - [x] `ExpressionParser.parseBlockExpression` の単一式再パースを廃止し、block 内 statement 列と末尾式を AST で保持する
  - [x] block expression 内の local declaration/scope を type inference に接続する
  - [x] `if`/`when`/`try` branch に `{ a; b; c }` を置いた場合の型と実行順序を回帰テスト化する

- [x] P5-48: `return` の制御フローを nested expression から正しく伝播させる（spec.md J6/J11/J12）
  - [x] BuildKIR で `returnExpr` を値式として潰さず、関数終端ジャンプとして lowering する
  - [x] `if`/`when`/`try` の分岐内部にある `return` を親関数の return へ接続する
  - [x] nested return 回帰ケース（`if` 内 return / `when` branch return）を追加する

- [x] P5-49: subject-less `when`（`when { ... }`）を Parser/Sema/KIR へ実装する（spec.md J5/J6/J9）
  - [x] `parseWhenExpression` が subject 必須になっている現実装を拡張し、subject 省略形を AST で表現する
  - [x] 分岐条件を `Boolean` 文脈で型検査し、exhaustiveness 規則へ接続する
  - [x] subject-less when の回帰ケース（guard 連鎖）を追加する

- [x] P5-50: `super` / qualified `this` 解決を実装する（spec.md J7/J9/J13.2）
  - [x] AST に `super` / `this@Label`（必要最小）を区別する参照ノードを追加する
  - [x] member 解決で supertype member lookup を実装し、override 先呼び出しに接続する
  - [x] super call の lowering/codegen（direct/special dispatch）を追加する

- [x] P5-51: `if/when` の branch 評価を eager `select` から制御フロー化へ移行する（spec.md J11/J12）
  - [x] `BuildKIRPass` の `ifExpr`/`whenExpr` lowering で全分岐を先に評価してしまう現実装を廃止する
...
- [x] P5-66: nullable/non-null スマートキャスト伝播を DataFlowState に統合する（spec.md J10/J10.1）
  - [x] `DataFlowState` に variable ごとの nullability-refined 型（`narrowedType`）を追加する
  - [x] `if (x != null)` / `x ?: return` / `!!` などのガードパターンを DataFlowAnalysis が認識し、then 分岐で `x` の型を non-null に narrow する
...
- [x] P5-67: compound assignment 演算子（`+=` / `-=` / `*=` / `/=` / `%=`）を parser/sema/KIR に実装する（spec.md J5/J9）
  - [x] Parser/AST に `compoundAssign` ノードを追加し、`+=` 〜 `%=` に対応するトークンを認識する
  - [x] Sema で `x += y` を `x = x.plus(y)`（または operator function）へ desugaring し、`val` への再代入を診断する
  - [x] `augmentedAssignment` operator（`plusAssign`/`minusAssign` 等）が定義されている型では `plusAssign` 優先で解決する
  - [x] KIR/codegen で compound assign を read-modify-write の IR シーケンスへ lowering する
  - [x] compound assign × member property / index 演算子（`a[i] += 1`）の組み合わせを正しく展開する
...
- [x] P5-71: `in` / `!in` 演算子と `contains` operator への desugaring を実装する（spec.md J9）
  - [x] Parser/AST で `inExpr`（`x in collection`）/ `notInExpr`（`x !in collection`）を認識する
  - [x] Sema で `x in c` → `c.contains(x)`、`x !in c` → `!c.contains(x)` へ desugaring する
  - [x] `when` branch の `in range` / `!in list` パターンを型検査・exhaustiveness に接続する（P5-83 と連携）
...
- [x] P5-74: `data class` の合成メンバ（`copy`/`componentN`/`equals`/`hashCode`/`toString`）を完成させる（spec.md J6/J14.3）
  - [x] DataEnumSealedSynthesis で `data class` の primary constructor 引数から `componentN()` を生成する
  - [x] `copy(param = value)` を named argument 付き constructor 呼び出しとして合成する
  - [x] `equals`（structual equality）/ `hashCode` / `toString` を field リストから合成し、既存 override と整合させる
  - [x] `==`（`equals` desugaring）/ destructuring で `componentN` を正しく呼ぶ回帰ケースを追加する
...
- [x] P5-83: `when` の型パターン（`is T` / `in range` / `!in collection`）と guard を完全実装する（spec.md J6/J9/J10）
  - [x] Parser/AST で `when` branch condition に `is T`（型チェック）/ `in expr`（`in` テスト）/ `!in expr`（`!in` テスト）を表現する
  - [x] `is T` branch 直後に smart cast（P5-66 連携）を適用する
  - [x] `in range` / `!in list` を P5-71 の `contains` desugaring 経由で評価する
  - [x] subject-less `when`（P5-49）と型パターンを組み合わせた guard 連鎖を型検査する
  - [x] sealed / enum × 型パターンの exhaustiveness チェックを接続する（P5-76/P5-78 連携）
  - [x] 型パターン・range テスト・`!in` を組み合わせた diff/golden ケースを追加する
...
- [x] P5-86: `@annotation` 構文を Parser/AST/Sema で保持し、use-site target を metadata に反映する（spec.md J6/J14.3）
  - [x] Parser/AST でアノテーション（`@Foo` / `@Foo(args)` / `@file:Foo`）を modifiers node に保持する
  - [x] Sema でアノテーション型を `class` シンボルとして解決し、引数の型を検証する
  - [x] use-site target（`@get:` / `@set:` / `@field:` 等）を property accessor へ付与する規則を実装する
  - [x] `metadata.bin` にアノテーション情報を保存・復元し、import 側から参照できるようにする
  - [x] アノテーション定義・付与・metadata 往復の基本 diff/golden ケースを追加する
...
- [x] P5-88 (runtime ABI): `Flow<T>` の runtime C ABI stub を Coroutine Runtime ABI タスクへ統合済み
  ...
  - [x] `flow { emit(x) }` を `Flow<T>` ビルダとして lowering し、emit point を suspension として扱う
  - [x] `collect { value -> ... }` を suspend 呼び出しのループ展開として lowering する
  - [x] backpressure なし hot skip / cold flow の基本動作を diff/golden ケースで追加する
...
- [x] P5-91: incremental compilation（変更ファイルのみ再コンパイル）の基盤を整備する（spec.md J1）
  - [x] `CompilerDriver` に入力ファイルの content hash / mtime を記録するキャッシュ層を追加する
  - [x] 変更なしファイルの Parse/AST/Sema 結果を `.kirbin` キャッシュから再利用する経路を実装する
  - [x] 依存グラフ（symbol 使用関係）を构築し、変更ファイルに依存するファイルを再コンパイル対象に追加する
  - [x] incremental ビルドと full ビルドの出力（object/executable）が byte 同一になることを検証する
  - [x] 10 ファイル構成でのインクリメンタル vs full の compile 時間を計測し、改善率を記録する
...
- [x] P5-93: multiline raw string（`"""..."""`）のエスケープなし文字列リテラルを完全実装する（spec.md J4）
  - [x] `"""` で囲われた生文字列トークンを Lexer で独立したトークン種別（`rawStringLiteral`）として扱う
  - [x] 内部改行・タブ・`\` をエスケープ不要として保持し、文字コード変換を行わない
  - [x] `trimIndent()` / `trimMargin()` の stdlib stub を追加し、common 用法を lowering で接続する
  - [x] raw string 内の `$` 補間（`${expr}`・`$name`）を通常 template と同等に処理する
  - [x] raw string リテラルの diff/golden ケース（multiline・補間・末尾空白除去）を追加する
...
- [x] P5-95: 数値リテラルの underscore/suffix/binary 形式と型強制を完全実装する（spec.md J4）
  - [x] `1_000_000` の underscore 区切りを lexer で処理し、値には影響しない
  - [x] `0b1010` binary リテラルを Kotlin 仕様どおりに受理する（`0o` 等は拒否・診断）
  - [x] `0xFF` / `0XDEADBEEF` hex リテラルの大文字小文字を正規化する
  - [x] `1L` / `1.0f` / `1.0` / `1.0F` の suffix を保持し、型推論で `Long`/`Float`/`Double` を確定する
  - [x] 範囲外リテラル（`Int` に入らない `999999999999`）の診断を追加する
  - [x] all-format の diff/golden ケース（binary・hex・underscore・suffix・範囲外）を追加する
...
- [ ] P5-96: `Nothing` 型の型推論・制御フロー Unreachable 統合を完成させる（spec.md J8/J10）
  ...
  - [ ] diff/golden ケースを追加する → `bash Scripts/generate_test_case.sh --from-registry Scripts/test_case_registry.json --task P5-96`
...
- [ ] P5-97: intersection type（`T & U`）と captured type を型システムに追加する（spec.md J8）
  ...
  - [ ] diff/golden ケースを追加する → `bash Scripts/generate_test_case.sh --from-registry Scripts/test_case_registry.json --task P5-97`
...
- [ ] P5-98: 型投影（`out T` / `in T` / `*`）の use-site variance を完全実装する（spec.md J8）
  ...
  - [ ] diff/golden ケースを追加する → `bash Scripts/generate_test_case.sh --from-registry Scripts/test_case_registry.json --task P5-98`
...
- [ ] P5-99: typealias の generic alias・循環検出・展開深度制限を完全実装する（spec.md J6/J8）
  ...
  - [ ] diff/golden ケースを追加する → `bash Scripts/generate_test_case.sh --from-registry Scripts/test_case_registry.json --task P5-99`
...
- [x] P5-100: `as`（unsafe cast）/ `as?`（safe cast）を型推論・KIR・runtime まで実装する（spec.md J9）
  - [x] Parser/AST に `castExpr`（`expr as Type`）/ `safeCastExpr`（`expr as? Type`）ノードを追加する
  - [x] `as` は cast 失敗時に `ClassCastException` を `outThrown` 経由で throw し、型を target type へ narrow する
  - [x] `as?` は cast 失敗時に `null` を返し、型を `TargetType?` へ narrow する
  - [x] KIR に cast チェック命令を追加し、codegen で runtime `kk_typecheck` 呼び出しへ lowering する
  - [x] `as` cast 後の smart cast（P5-66 連携）伝播を実装する
  - [x] diff/golden ケースを追加する → `bash Scripts/generate_test_case.sh --from-registry Scripts/test_case_registry.json --task P5-100`
...
- [ ] P5-101: `is` / `!is` 型検査と smart cast を完全実装する（spec.md J9/J10）
  ...
  - [ ] diff/golden ケースを追加する → `bash Scripts/generate_test_case.sh --from-registry Scripts/test_case_registry.json --task P5-101`
...
- [ ] P5-102: 演算子の優先順位テーブルを Kotlin 仕様完全準拠で実装する（spec.md J5）
  ...
  - [ ] diff/golden ケースを追加する → `bash Scripts/generate_test_case.sh --from-registry Scripts/test_case_registry.json --task P5-102`
...
- [ ] P5-103: bitwise / shift 演算子（`and`/`or`/`xor`/`inv`/`shl`/`shr`/`ushr`）を infix 関数として実装する（spec.md J9）
  ...
  - [ ] diff/golden ケースを追加する → `bash Scripts/generate_test_case.sh --from-registry Scripts/test_case_registry.json --task P5-103`
...
- [ ] P5-104: `@Label` / `return@label` / `break@label` / `continue@label` を完全実装する（spec.md J5/J6）
  ...
  - [ ] diff/golden ケースを追加する → `bash Scripts/generate_test_case.sh --from-registry Scripts/test_case_registry.json --task P5-104`
...
- [ ] P5-105: `when` の複数条件ブランチ（`,` 区切り）と exhaustive 診断精度を強化する（spec.md J5/J6）
  ...
  - [ ] diff/golden ケースを追加する → `bash Scripts/generate_test_case.sh --from-registry Scripts/test_case_registry.json --task P5-105`
...
- [ ] P5-106: `try` を式として使う場合の型推論と `finally` 影響を実装する（spec.md J6/J11）
  ...
  - [ ] diff/golden ケースを追加する → `bash Scripts/generate_test_case.sh --from-registry Scripts/test_case_registry.json --task P5-106`
...
- [ ] P5-107: `do-while` の condition スコープと `break`/`continue`・初回実行を完全実装する（spec.md J5/J6）
  ...
  - [ ] diff/golden ケースを追加する → `bash Scripts/generate_test_case.sh --from-registry Scripts/test_case_registry.json --task P5-107`
...
- [ ] P5-108: top-level property の backing field・getter/setter・初期化順序を front-to-back で実装する（spec.md J6/J7）
  ...
  - [ ] diff/golden ケースを追加する → `bash Scripts/generate_test_case.sh --from-registry Scripts/test_case_registry.json --task P5-108`
...
- [ ] P5-109: `const val` のコンパイル時定数畳み込みを実装する（spec.md J6/J7）
  ...
  - [ ] diff/golden ケースを追加する → `bash Scripts/generate_test_case.sh --from-registry Scripts/test_case_registry.json --task P5-109`
  - **完了条件**: `const val MAX = 100; if (x > MAX) ...` が `if (x > 100)` と同等にコンパイルされる

- [x] P5-110: `lateinit var` の初期化チェックと `isInitialized` を実装する（spec.md J6/J7）
  - [x] `lateinit var` を Sema で認識し、nullable でないこと・reference type 制約を検証する
  - [x] 未初期化 `lateinit var` へのアクセス時に `UninitializedPropertyAccessException` を `outThrown` 経由で throw する
  - [x] `::x.isInitialized` を property reference の特別属性として Sema/KIR に接続する
...
- [ ] P5-111: `object` declaration（singleton）の lazy 初期化と init block 実行順序を実装する（spec.md J6/J7）
  ...
  - [ ] diff/golden ケースを追加する → `bash Scripts/generate_test_case.sh --from-registry Scripts/test_case_registry.json --task P5-111`
...
- [ ] P5-112: abstract class / abstract member の制約と override 強制を実装する（spec.md J6/J7）
  ...
  - [ ] diff/golden ケースを追加する → `bash Scripts/generate_test_case.sh --from-registry Scripts/test_case_registry.json --task P5-112`
...
- [ ] P5-113: `interface` default method（body あり fun in interface）を front-to-back で実装する（spec.md J6/J7/J13.2）
  ...
  - [ ] diff/golden ケースを追加する → `bash Scripts/generate_test_case.sh --from-registry Scripts/test_case_registry.json --task P5-113`
  - **完了条件**: interface default method が override されない場合に default 実装が呼ばれる

- [x] P5-114: 多重インターフェース実装と diamond override の解決規則を実装する（spec.md J7/J13.2）
...
- [ ] P5-115: `open` / `final` / `override` 修飾子の継承制約を完全実装する（spec.md J6/J7）
  ...
  - [ ] diff/golden ケースを追加する → `bash Scripts/generate_test_case.sh --from-registry Scripts/test_case_registry.json --task P5-115`
...
- [ ] P5-116: `data object` / anonymous object の型と等値比較を実装する（spec.md J6）
  ...
  - [ ] diff/golden ケースを追加する → `bash Scripts/generate_test_case.sh --from-registry Scripts/test_case_registry.json --task P5-116`
...
- [ ] P5-117: constructor の `init` block と primary constructor property の初期化順序を保証する（spec.md J7）
  ...
  - [ ] diff/golden ケースを追加する → `bash Scripts/generate_test_case.sh --from-registry Scripts/test_case_registry.json --task P5-117`
...
- [ ] P5-119: infix 関数宣言（`infix fun`）の構文と解決を実装する（spec.md J9）
  ...
  - [ ] diff/golden ケースを追加する → `bash Scripts/generate_test_case.sh --from-registry Scripts/test_case_registry.json --task P5-119`
  - **完了条件**: `1 to "one"` が `Pair(1, "one")` に、カスタム infix 関数が正しい優先順位で評価される

- [x] P5-120: `operator fun` の全標準 operator を網羅し診断を整備する（spec.md J9）
  - [x] 標準 operator 名（`plus`/`minus`/`times`/`div`/`rem`/`unaryPlus`/`unaryMinus`/`not`/`inc`/`dec`/`rangeTo`/`rangeUntil`/`contains`/`get`/`set`/`invoke`/`iterator`/`hasNext`/`next`/`component1..N`/`compareTo`/`equals`）を全列挙する
  - [x] operator 名と引数・戻り値の型制約を Sema で検証する（例: `inc()` は receiver 型を返す）
  - [x] `operator` 修飾子なしで operator 名の関数を演算子として使おうとした場合に診断する
  - [x] `inc`/`dec`（prefix/postfix `++`/`--`）の pre/post セマンティクスを正しく lowering する
...
- [ ] P5-121: function type / lambda の `it`・型省略・destructuring を完全実装する（spec.md J9/J12）
  ...
  - [ ] diff/golden ケースを追加する → `bash Scripts/generate_test_case.sh --from-registry Scripts/test_case_registry.json --task P5-121`
...
- [ ] P5-122: extension property を型システム・member dispatch に統合する（spec.md J7/J9）
  ...
  - [ ] diff/golden ケースを追加する → `bash Scripts/generate_test_case.sh --from-registry Scripts/test_case_registry.json --task P5-122`
...
- [ ] P5-123: getter/setter 内で backing field を参照する `field` キーワードを実装する（spec.md J7）
  ...
  - [ ] diff/golden ケースを追加する → `bash Scripts/generate_test_case.sh --from-registry Scripts/test_case_registry.json --task P5-123`
...
- [ ] P5-124: `provideDelegate` operator と `KProperty<*>` stub を完全連携させる（spec.md J7/J9）
  ...
  - [ ] diff/golden ケースを追加する → `bash Scripts/generate_test_case.sh --from-registry Scripts/test_case_registry.json --task P5-124`
...
- [ ] P5-125: 複数 upper bound（`where T : A, T : B`）と F-bound（`T : Comparable<T>`）を完全実装する（spec.md J8）
  ...
  - [ ] diff/golden ケースを追加する → `bash Scripts/generate_test_case.sh --from-registry Scripts/test_case_registry.json --task P5-125`
...
- [ ] P5-127: variance（`out T`/`in T`）の declaration-site 制約違反診断を実装する（spec.md J8）
  ...
  - [ ] diff/golden ケースを追加する → `bash Scripts/generate_test_case.sh --from-registry Scripts/test_case_registry.json --task P5-127`
...
- [ ] P5-128: `reified` inline 関数での `T::class` / `typeOf<T>()` を完全実装する（spec.md J12.2）
  ...
  - [ ] diff/golden ケースを追加する → `bash Scripts/generate_test_case.sh --from-registry Scripts/test_case_registry.json --task P5-128`
...
- [ ] P5-129: generic lambda と SAM conversion（functional interface）を実装する（spec.md J8/J12）
  ...
  - [ ] diff/golden ケースを追加する → `bash Scripts/generate_test_case.sh --from-registry Scripts/test_case_registry.json --task P5-129`
...
- [ ] P5-130: platform type（nullability 不明型 `T!`）の扱いを実装する（spec.md J8）
  ...
  - [ ] diff/golden ケースを追加する → `bash Scripts/generate_test_case.sh --from-registry Scripts/test_case_registry.json --task P5-130`
...
- [ ] P5-131: nullable receiver（`T?.foo()`）拡張関数を Sema で解決する（spec.md J7/J9）
  ...
  - [ ] diff/golden ケースを追加する → `bash Scripts/generate_test_case.sh --from-registry Scripts/test_case_registry.json --task P5-131`
...
- [ ] P5-132: nullable な型引数（`List<String?>`）と non-null 型引数（`List<String>`）の区別を実装する（spec.md J8）
  ...
  - [ ] diff/golden ケースを追加する → `bash Scripts/generate_test_case.sh --from-registry Scripts/test_case_registry.json --task P5-132`
...
- [x] P5-133 (runtime ABI): `withContext` / Dispatchers の runtime C ABI stub を Coroutine Runtime ABI タスクへ統合済み
  ...
  - [x] `withContext` body を新たな coroutine context で実行し、完了後に元 context へ戻る lowering を実装する
  - [x] diff/golden ケースを追加する → `bash Scripts/generate_test_case.sh --from-registry Scripts/test_case_registry.json --task P5-133`
...
- [x] P5-134 (runtime ABI): `Channel<T>` の runtime C ABI stub を Coroutine Runtime ABI タスクへ統合済み
  ...
  - [x] `channel.send(value)` / `channel.receive()` を suspension point として KIR に lowering する
  - [x] unbuffered channel の rendezvous semantics（sender が receiver を待つ）を runtime で実装する
  - [x] diff/golden ケースを追加する → `bash Scripts/generate_test_case.sh --from-registry Scripts/test_case_registry.json --task P5-134`
...
- [x] P5-135 (runtime ABI): `async`/`await`（`Deferred<T>`）の runtime C ABI stub を Coroutine Runtime ABI タスクへ統合済み
  ...
  - [x] `async { }` が `Deferred<T>` を返す lowering を実装する
  - [x] `deferred.await()` を suspension point として実装し、結果型 `T` を推論する
  - [x] diff/golden ケースを追加する → `bash Scripts/generate_test_case.sh --from-registry Scripts/test_case_registry.json --task P5-135`
...
- [ ] P5-136: coroutine cancellation と `CancellationException` の伝播を実装する（spec.md J17）
  ...
  - [ ] diff/golden ケースを追加する → `bash Scripts/generate_test_case.sh --from-registry Scripts/test_case_registry.json --task P5-136`
...
- [x] P5-137: スコープ関数（`let`/`run`/`apply`/`also`/`with`）を extension として実装する（spec.md J9/J12）
  - [x] `let`/`run`/`apply`/`also`/`with` を stdlib extension function stub として定義する
  - [x] 各スコープ関数の receiver / lambda parameter / 戻り値の型規則を正確に実装する
  ...
  - [x] null 安全 `let`（`nullable?.let { ... }`）のショートサーキット動作を確認する
  - [x] diff/golden ケースを追加する → `bash Scripts/generate_test_case.sh --from-registry Scripts/test_case_registry.json --task P5-137`
...
- [ ] P5-138: `buildString`/`buildList`/`buildMap` DSL builder を実装する（spec.md J9/J12）
  ...
  - [ ] diff/golden ケースを追加する → `bash Scripts/generate_test_case.sh --from-registry Scripts/test_case_registry.json --task P5-138`
...
- [ ] P5-139: `Sequence<T>` と lazy evaluation chain（`asSequence`/`map`/`filter`/`toList`）を実装する
  ...
  - [ ] diff/golden ケースを追加する → `bash Scripts/generate_test_case.sh --from-registry Scripts/test_case_registry.json --task P5-139`
...
- [ ] P5-140: `expect`/`actual` 宣言を parser/sema/metadata で扱う（spec.md J14 / Kotlin MPP）
  ...
  - [ ] diff/golden ケースを追加する → `bash Scripts/generate_test_case.sh --from-registry Scripts/test_case_registry.json --task P5-140`
  - **完了条件**: `expect fun platform()` に対する `actual fun platform()` が正しくリンクされ動作する

- [x] P5-141: file-level annotation（`@file:JvmName`・`@file:Suppress`）と package-level 制約を実装する（spec.md J6）
  - [x] `@file:AnnotationName` を Parser/AST でファイルレベル annotation として保持する
  - [x] `@file:Suppress("CODE")` をファイル全体への診断抑制として Sema で適用する
  - [x] `@file:JvmName("...")` を ABI / metadata で保持し、他モジュールから参照できるようにする
  - [x] diff/golden ケースを追加する → `bash Scripts/generate_test_case.sh --from-registry Scripts/test_case_registry.json --task P5-141`
  - **完了条件**: `@file:Suppress("UNUSED")` がファイル全体の UNUSED 警告を抑制する

---

## P5 (Spec Gap Backlog) — 実用 Kotlin プログラム完全対応拡張
...
- [ ] P5-142: stdlib String メンバ関数の最小スタブを実装する（spec.md J15/J16）
  - [ ] runtime に `kk_string_length` / `kk_string_substring` / `kk_string_indexof` / `kk_string_split` / `kk_string_trim` / `kk_string_replace` / `kk_string_starts_with` / `kk_string_ends_with` の C ABI を追加する
  - [ ] compiler 側で `String` 型の member/extension call を上記 ABI へ解決する stub を追加する（既存 `kk_string_concat`/`kk_string_compareTo` のパターンを踏襲）
  - [ ] `String.length` プロパティアクセスを property getter lowering と接続する
  - [ ] String 操作の diff/golden 回帰ケースを追加する（`.length`・`.substring`・`.indexOf`・`.split`・`.replace`）
  - **完了条件**: `"hello".length == 5` / `"hello".substring(1, 3) == "el"` が `kotlinc` と同一出力になる

### 🔢 stdlib — 数値型変換

- [ ] P5-143: 数値型変換関数（`.toInt()`, `.toLong()`, `.toDouble()`, `.toString()` 等）を実装する（spec.md J8/J15）
  - [ ] プリミティブ間変換（`Int.toDouble()`, `Int.toLong()`, `Double.toInt()`, `Long.toInt()`, `Long.toDouble()`, `Float.toInt()` 等）を runtime stub として追加する
  - [ ] `String.toInt()` / `String.toLong()` / `String.toDouble()` を実装し、変換失敗時は `NumberFormatException` を投げる
  - [ ] `Any.toString()` の汎用ディスパッチを整備し、各 primitive 型の文字列化を統一する（現状 `kk_println_any` 内部のみ）
  - [ ] `Int.toString(radix: Int)` による基数変換を追加する
  - [ ] 数値変換の diff/golden 回帰ケースを追加する（primitive 間変換・String 変換・toString）
  - **完了条件**: `42.toLong() * 2L` / `"123".toInt() + 1` が `kotlinc` と同一出力になる

### ⚠️ runtime — 例外型階層

- [ ] P5-144: 例外型階層の最小実装を行う（spec.md J11.3/J13.3/J15）
  - [ ] `Exception` / `RuntimeException` / `IllegalArgumentException` / `IllegalStateException` / `NullPointerException` / `IndexOutOfBoundsException` / `ClassCastException` / `NumberFormatException` / `NotImplementedError` を named type として型システムに登録する
  - [ ] runtime に各例外型の constructor（`kk_illegal_argument_exception_new` / `kk_null_pointer_exception_new` 等）を追加する
  - [ ] `catch (e: SpecificType)` の型マッチディスパッチを実装する（`kk_throwable_instanceof` で型チェック）
  - [ ] `!!` → NPE、`as` 失敗 → ClassCastException、配列 bounds check 失敗 → IOOBE と各例外型を接続する
  - [ ] `String.toInt()` 変換失敗 → NumberFormatException と接続する
  - [ ] 例外型マッチの diff/golden 回帰ケースを追加する（型付き catch・型不一致の再 throw）
  - **完了条件**: `catch (e: IllegalArgumentException)` が `IllegalStateException` を捕捉せず、型階層に沿ったマッチが動作する

### 📦 stdlib — コレクション操作

- [ ] P5-145: `Iterable<T>` 拡張関数（`.map`, `.filter`, `.forEach` 等）の最小スタブを実装する（spec.md J15）
  - [ ] `.map`, `.filter`, `.forEach`, `.flatMap`, `.reduce`, `.fold`, `.any`, `.all`, `.none` を `Iterable<T>` extension として stub 実装する
  - [ ] `.first()` / `.last()` / `.firstOrNull()` / `.lastOrNull()` を追加する
  - [ ] `.sorted()` / `.sortedBy { }` / `.reversed()` を追加する
  - [ ] `.toList()` / `.toSet()` / `.toMutableList()` 変換を追加する
  - [ ] lambda 引数の型推論（`list.map { it + 1 }`）が P5-121 連携で機能することを確認する
  - [ ] `listOf(1,2,3).filter { it > 1 }.map { it * 2 }` の diff/golden ケースを追加する
  - **完了条件**: `listOf(1,2,3).map { it * 2 }.filter { it > 2 }` が `[4, 6]` を返し `kotlinc` と一致する

- [ ] P5-148: `MutableList` / `MutableMap` の変更操作を実装する（spec.md J15）
  - [ ] `MutableList.add()` / `.remove()` / `.removeAt()` / `.clear()` / `.addAll()` を runtime stub に追加する
  - [ ] `MutableMap.put()` / `.remove()` / `.clear()` / `.putAll()` を runtime stub に追加する
  - [ ] `mutableListOf(...)` / `mutableMapOf(...)` ファクトリ関数を追加する
  - [ ] P5-84 の read-only `List<T>`/`Map<K,V>` を `MutableList`/`MutableMap` 継承関係で整理する
  - [ ] mutable collection の変更操作 diff/golden ケースを追加する
  - **完了条件**: `val list = mutableListOf(1); list.add(2); println(list.size)` が `2` を出力し `kotlinc` と一致する

### 🖨️ stdlib — 基本 I/O

- [ ] P5-146: `print()` 関数（改行なし出力）を実装する（spec.md J15）
  - [ ] runtime に `kk_print_any` / `kk_print_int` / `kk_print_long` / `kk_print_float` / `kk_print_double` / `kk_print_char` / `kk_print_bool` を追加する（`kk_println_*` パターンを踏襲）
  - [ ] compiler で `print(...)` 呼び出しを上記 ABI へ解決する（`println` の解決経路を参照）
  - [ ] `print()` の diff/golden 回帰ケースを追加する
  - **完了条件**: `print("Hello "); println("World")` が `Hello World\n` を出力し `kotlinc` と一致する

### 🎯 stdlib — Pair / Triple

- [ ] P5-147: `Pair<A,B>` / `Triple<A,B,C>` クラスと `to` infix 関数を実装する（spec.md J6/J15）
  - [ ] `Pair(first: A, second: B)` を stdlib stub として data-class 相当で定義する（`component1`/`component2`/`toString`/`copy` 合成）
  - [ ] `Triple(first: A, second: B, third: C)` も同様に定義する
  - [ ] `to` infix 関数（`a to b` → `Pair(a, b)`）を P5-119（infix 関数）連携で実装する
  - [ ] destructuring（P5-82 連携）で `val (a, b) = pair` を `component1()`/`component2()` 展開にする
  - [ ] `mapOf(1 to "one", 2 to "two")` の diff/golden ケースを追加する
  - **完了条件**: `val (x, y) = 1 to 2; println("$x $y")` が `1 2` を出力し `kotlinc` と一致する

### 🔗 class delegation

- [ ] P5-149: class delegation（`class A : B by impl`）を front-to-back で実装する（spec.md J6/J7/J13.2）
  - [ ] Parser/AST で `classDecl` の superType 節にある `by delegateExpr` 句を保持する
  - [ ] Sema で delegate 対象の interface member を自動 forward 実装として生成する（`impl.method(args)` 呼び出しを body とする合成 member）
  - [ ] delegated member と明示 override の共存を検証し、override 優先ルールを実装する
  - [ ] compiler-generated delegation と vtable/itable dispatch（P5-25）を接続する
  - [ ] class delegation の diff/golden 回帰ケースを追加する
  - **完了条件**: `class LoggingList<T>(private val inner: MutableList<T>) : MutableList<T> by inner` が inner の全メソッドを委譲して動作する

---

### 🛠️ stdlib — 標準ユーティリティ

- [ ] P5-150: 標準ユーティリティ関数スタブを実装する（spec.md J15）
  - [ ] `require(value: Boolean)` / `require(value: Boolean, lazyMessage: () -> Any)` → 失敗時 `IllegalArgumentException` throw
  - [ ] `requireNotNull(value: T?)` → 失敗時 `IllegalArgumentException` throw、成功時 `T` を返す
  - [ ] `check(value: Boolean)` / `checkNotNull(value: T?)` → 失敗時 `IllegalStateException` throw
  - [ ] `error(message: Any)` → `IllegalStateException` throw
  - [ ] `TODO(reason: String = "An operation is not implemented.")` → `NotImplementedError` throw
  - [ ] `readLine()` / `readln()` → 標準入力から 1 行を `String?`/`String` として読み取る（runtime に `kk_readline` を追加）
  - [ ] `maxOf(a, b)` / `minOf(a, b)` → 各プリミティブ型オーバーロードを stub 実装
  - [ ] `repeat(times: Int, action: (Int) -> Unit)` → loop 展開として lowering
  - [ ] P5-144（例外型階層）が前提となることを確認する
  - [ ] 各関数の diff/golden 回帰ケースを追加する
  - **完了条件**: `require(x > 0) { "must be positive" }` が x≦0 で `IllegalArgumentException` を throw し `kotlinc` と一致する

### 📌 annotation class 宣言

- [ ] P5-151: `annotation class` 宣言（カスタムアノテーション型定義）を実装する（spec.md J6.3/J7.1）
  - [ ] `annotation class Foo(val x: String = "default")` を Parser/AST で専用ノード化し `SymbolKind.annotationClass` に登録する
  - [ ] primary constructor パラメータは `val` のみ、型は primitive/String/enum/KClass/annotation/array に制限し違反時に診断する
  - [ ] Sema でアノテーション適用時に annotation class シンボルを参照して引数型を検証する
  - [ ] `annotation class` の基本 diff/golden ケースを追加する
  - **完了条件**: `@MyAnnotation("hello") fun foo()` が `MyAnnotation` を annotation class として解決し metadata に保存される

### 📚 --emit library モード

- [ ] P5-152: `--emit library` モード（`.kklib` 生成）を実装する（spec.md J1.1/J14）
  - [ ] `CompilerOptions.emitMode == .library` 時のパイプラインを整備し、object ファイル・metadata.bin・inline-kir を生成して `.kklib/` ディレクトリへ出力する
  - [ ] manifest.json に `formatVersion`/`moduleName`/`kotlinLanguageVersion`/`target`/`objects`/`metadata`/`inlineKIRDir` を書き出す（P5-54 の検証スキーマを逆用）
  - [ ] `--emit library` で生成した `.kklib` を `-I` で consumer に渡す E2E テストを追加する
  - **完了条件**: `kswiftc --emit library lib.kt -o mylib.kklib` が生成した `.kklib` を consumer が正しくリンクできる

### 🔧 inline 修飾子

- [ ] P5-153: `crossinline` / `noinline` ラムダパラメータ修飾子を実装する（spec.md J6.3/J12.2）
  - [ ] `noinline` modifier を Parser/AST で保持し、InlineLowering で当該ラムダパラメータの inline 展開をスキップする
  - [ ] `crossinline` modifier を保持し、non-local return を含む lambda を `crossinline` 位置で使用した場合に `KSWIFTK-SEMA-*` 診断を出す
  - [ ] `noinline`/`crossinline` の diff/golden 回帰ケースを追加する
  - **完了条件**: `inline fun foo(noinline block: () -> Unit)` で `block` が展開されず関数オブジェクトとして渡される

### 🌐 external fun (FFI)

- [ ] P5-154: `external fun` 宣言（ネイティブ FFI）を実装する（spec.md J6.3）
  - [ ] `external fun` を Sema で body なし関数として許可し、ABI 互換な C 関数シンボルへの direct call を codegen で生成する
  - [ ] `@CName("c_func")` アノテーション連携でシンボル名を上書き可能にする（P5-154 のスコープ内）
  - [ ] external 関数の diff/golden ケースを追加する
  - **完了条件**: `external fun c_sqrt(x: Double): Double` が C stdlib の `sqrt` を呼び出して正しい値を返す

### 🔍 Parser — `<T>` vs `<` 曖昧性

- [ ] P5-155: `<T>` vs `<` 曖昧性解消アルゴリズムを Parser に実装する（spec.md J5.5）
  - [ ] `canStartTypeArguments` 関数（N=32 先読み）を Parser に追加する
  - [ ] `foo<Int>(x)` を generic 呼び出しとして、`a < b` を比較として正しく区別する
  - [ ] 型引数付き generic call の parser golden ケースを追加する
  - **完了条件**: `val x = foo<Int>(1) + bar<Long>(2L)` が型引数付き呼び出しとして正しく parse される

### 🏷️ Lexer — backticked identifiers

- [ ] P5-156: backticked identifiers（`` `reserved` ``）を Lexer に実装する（spec.md J4.1）
  - [ ] Lexer で `` ` `` で囲まれた任意のテキストを `backtickedIdentifier` トークンとして認識する
  - [ ] backticked identifier を通常の識別子と同等に Sema で扱う
  - [ ] `` fun `should return true`() `` 形式のテスト関数名の lexer golden を追加する
  - **完了条件**: `` val `class` = 1 `` が予約語エラーなく parse・コンパイルされる

### 🎰 enum エントリコンストラクタ引数

- [ ] P5-157: enum entry コンストラクタ引数を実装する（spec.md J6.2/J7）
  - [ ] `enum class Color(val rgb: Int) { RED(0xFF0000), GREEN(0x00FF00), BLUE(0x0000FF) }` を Parser/AST で扱う
  - [ ] enum primary constructor の引数を entry 宣言時に解決する（constructor overload resolution と接続）
  - [ ] entry body（abstract fun override 等）の解析と合成メンバ生成を追加する
  - [ ] データ付き enum の diff/golden ケースを追加する
  - **完了条件**: `Color.RED.rgb == 0xFF0000` が動作し `kotlinc` と同一出力になる

### 🎯 stdlib — takeIf / takeUnless

- [ ] P5-158: `takeIf` / `takeUnless` スコープ関数を実装する（spec.md J9/J15）
  - [ ] `T.takeIf(predicate: (T) -> Boolean): T?` → 条件 true なら `this`、false なら `null`
  - [ ] `T.takeUnless(predicate: (T) -> Boolean): T?` → 逆
  - [ ] P5-137（スコープ関数）の stub パターンを踏襲して実装する
  - [ ] `takeIf`/`takeUnless` の diff/golden ケースを追加する
  - **完了条件**: `"hello".takeIf { it.length > 3 }?.uppercase()` が `"HELLO"` を返し `kotlinc` と一致する

---

### 🗑️ GC / Runtime（将来対応）

- [ ] P5-159: GC write barrier 挿入（`kk_write_barrier`）を codegen に実装する（spec.md J16.1）
  - [ ] 参照型フィールドへの書き込み時に `kk_write_barrier(owner, fieldAddr)` を codegen で挿入する
  - [ ] write barrier 挿入の E2E テスト（GC 圧力下での正確性確認）を追加する
  - **完了条件**: 参照型フィールド書き込みが全て write barrier を経由し、GC が cycles を正しく追跡できる

- [ ] P5-160: `gcDescriptor` per-type 生成（精密 GC スキャン）を実装する（spec.md J16.1）
  - [ ] `KTypeInfo.gcDescriptor` にフィールドの参照/非参照情報をビットマップで埋め込む
  - [ ] codegen でクラスごとの `gcDescriptor` を生成し `KTypeInfo` に接続する
  - [ ] 精密スキャンの GC 回帰テストを追加する
  - **完了条件**: GC がヒープオブジェクト内の参照フィールドのみをスキャンし、primitive フィールドをスキップする

### ⚡ Backend — 最適化レベル

- [ ] P5-161: 最適化レベル（O0/O1/O2/O3）を LLVM backend に連携させる（spec.md J1.1/J15.2）
  - [ ] `CompilerOptions.optimizationLevel` を `LLVMCAPIBackend` の `LLVMSetModuleIdentifier` / PassManager 呼び出しに伝播させる
  - [ ] `clang` 呼び出しに `-O0`/`-O1`/`-O2`/`-O3` フラグを接続する（synthetic C backend）
  - [ ] 最適化レベル別の出力サイズ・速度ベンチを追加する
  - **完了条件**: `kswiftc -O2` が `-O0` より小さい object を生成し、コードが正しく動作する

### 🐚 Lexer — shebang サポート

- [ ] P5-162: shebang（`#!`）の Lexer サポートを実装する（spec.md J4.1）
  - [ ] スクリプトファイル先頭の `#!...` 行をトリビアとして処理し、字句解析に影響させない
  - [ ] shebang 付きスクリプトの lexer golden ケースを追加する
  - **完了条件**: `#!/usr/bin/env kswiftc` を先頭に持つファイルが正常にコンパイルされる

### 🔄 Coroutines — CoroutineContext 型階層

- [ ] P5-163: `CoroutineContext` 型階層と合成演算子を実装する（spec.md J17.1）
  - [ ] `CoroutineContext` / `CoroutineContext.Element` / `CoroutineContext.Key<E>` の型定義を stdlib stub として追加する
  - [ ] `context1 + context2` 合成演算子（`plus` operator）を実装する
  - [ ] P5-89（structured concurrency）の `coroutineScope { }` と接続する
  - [ ] CoroutineContext 合成の E2E ケースを追加する
  - **完了条件**: `coroutineScope(CoroutineName("test") + Dispatchers.IO) { }` が正しく context を合成して実行される

- [ ] P5-164: `KKContinuation` プロトコルと `resumeWith`/`suspendCoroutine` を実装する（spec.md J17.1）
  - [ ] `suspendCoroutine { cont -> cont.resume(value) }` / `suspendCancellableCoroutine` のユーザーレベル API を追加する
  - [ ] `Result<T>` 型（`success`/`failure`）を stdlib stub として定義する
  - [ ] continuation の `resume(value)`/`resumeWithException(e)` を lowering/runtime に接続する
  - [ ] `suspendCoroutine` を使った手動 suspend 制御の E2E ケースを追加する
  - **完了条件**: `suspendCoroutine<Int> { cont -> cont.resume(42) }` が `42` を返す

### 📑 Contracts

- [ ] P5-165: Kotlin contracts（`contract { }` ブロック）の最小実装を追加する（spec.md J10）
  - [ ] `contract { }` ブロックを Parser/AST で保持し（エラーにしない）、`callsInPlace`/`returns`/`returnsNotNull` エフェクトを AST に記録する
  - [ ] `callsInPlace(block, InvocationKind.EXACTLY_ONCE)` を DataFlow が認識し、lambda 内変数初期化を "definite assignment" として扱う
  - [ ] `returns(true) implies (x != null)` を DataFlowState の smart cast に接続する
  - [ ] contracts を使った smart cast 改善の diff/golden ケースを追加する
  - **完了条件**: `contract { callsInPlace(block, EXACTLY_ONCE) }` により lambda 内での `val` 初期化が definite assignment として認識される

---

## 優先度マトリクス（P5-142〜P5-165）

| 優先度 | 番号 | 理由 |
|---|---|---|
| 🔴 P0 ブロッカー | P5-142/143/144 | String 操作・数値変換・例外型が無いと実用プログラムが動かない |
| 🔴 P0 ブロッカー | P5-145/146/147/148 | コレクション操作・print・Pair・MutableList が必須 |
| 🟠 P1 高 | P5-149/150/151 | class delegation・標準ユーティリティ・annotation class |
| 🟠 P1 高 | P5-152/153/155/156 | --emit library・crossinline/noinline・型引数曖昧性・backtick |
| 🟡 P2 中 | P5-154/157/158 | external fun・enum entry 引数・takeIf/takeUnless |
| 🟢 P3 低 | P5-159/160/161 | GC write barrier・gcDescriptor・最適化レベル |
| 🟢 P3 低 | P5-162/163/164/165 | shebang・CoroutineContext・suspendCoroutine・contracts |
...
P5 タスクの diff/golden テストケース追加は `Scripts/test_case_registry.json` で一元管理する。
各タスクの「diff/golden ケースを追加する」サブタスクは、以下のワークフローで一括処理できる。
...
| パス | 説明 |
|---|---|
| `Scripts/test_case_registry.json` | 全 P5 タスクのテストケース定義（タスク ID・カテゴリ・テンプレートパス） |
...
以下のカテゴリで関連タスクのテストを一括生成できる:

| カテゴリ | 対象タスク | テスト数 |
|---|---|---|
| `lexer` | P5-93, P5-94, P5-95 | 4 |
| `types` | P5-96, P5-97, P5-98, P5-99 | 7 |
| `expressions` | P5-100, P5-101, P5-102, P5-103, P5-104 | 10 |
| `control_flow` | P5-105, P5-106, P5-107 | 5 |
| `declarations` | P5-108, P5-109, P5-110, P5-111 | 7 |
| `class_object` | P5-112, P5-113, P5-114, P5-115, P5-116, P5-117 | 11 |
| `functions` | P5-118, P5-119, P5-120, P5-121 | 7 |
| `properties` | P5-122, P5-123, P5-124 | 5 |
| `generics` | P5-125, P5-126, P5-127, P5-128, P5-129 | 7 |
| `null_safety` | P5-130, P5-131, P5-132 | 3 |
| `coroutines` | P5-133, P5-134, P5-135, P5-136 | 4 |
| `stdlib` | P5-137, P5-138, P5-139 | 4 |
| `multiplatform` | P5-140, P5-141 | 2 |
