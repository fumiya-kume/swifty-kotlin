...
- [~] STDLIB-GAP-PH2: `kotlin.collections` / `kotlin.sequences` / `kotlin.ranges` の未対応を潰す（IntRange `toSet()` は runtime 接続済み。残: sortedSet/sortedMap/toSortedMap/toSortedSet/constrainTo/Sequence gap 等）
...
- [x] STDLIB-GAP-PH3: `kotlin.io`（common） / `kotlin.time` / `kotlin.concurrent` / `kotlin.concurrent.atomics` の未対応を潰す
...
- [x] STDLIB-TIME-TYPE-003: `Clock` インターフェースの実装
...
- [x] STDLIB-COMP-FN-022: `maxOf` 関数の実装（Long版、vararg）
...
- [x] STDLIB-COMP-FN-048: `minOf` 関数の実装（Short版、3引数）
- [x] STDLIB-COMP-FN-049: `minOf` 関数の実装（Short版、vararg）
...
- [x] STDLIB-COMP-FN-058: `naturalOrderComparator` 関数の実装
...
- [~] STDLIB-CORO-003: `kotlin.coroutines` の一部ランタイム経路をセマフォ待機から脱却する。`kk_with_context` は suspend-aware continuation 経路へ移行済み。残: `RuntimeAsyncTask.awaitResult`, `RuntimeJobHandle.join`, Channel send/receive, Sequence builder( `sequence`, `iterator` ) の待機部。
...
- [x] STDLIB-CINTEROP-INTERNAL-TYPE-004: `kotlinx.cinterop.internal.CGlobalAccess` annotation を追加する
- [x] STDLIB-CINTEROP-INTERNAL-TYPE-005: `kotlinx.cinterop.internal.ConstantValue` object を追加する
